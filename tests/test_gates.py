...
def test_is_unitary_helper():
    """Tests the internal _is_unitary helper function."""
    # Unitary
    assert gates._is_unitary(gates.H.matrix)
    assert gates._is_unitary(gates.CNOT.matrix)

    # Non-unitary
    assert not gates._is_unitary(np.array([[1, 1], [1, 1]]))
    assert not gates._is_unitary(np.array([[1, 0], [0, 2]]))

    # Non-square
    assert not gates._is_unitary(np.array([[1, 0, 0], [0, 1, 0]]))